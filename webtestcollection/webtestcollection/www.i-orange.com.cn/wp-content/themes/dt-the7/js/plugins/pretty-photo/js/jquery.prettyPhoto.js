	...
	$.fn.prettyPhoto = function(pp_settings) {
		...
		*/
		$.prettyPhoto.initialize = function() {
			...
			// New stuff
			var data_share_buttons = $(this).attr('data-pretty-share');
			pp_data_share_buttons = data_share_buttons ? data_share_buttons.split(',') : false;
			pp_data_permalinks = (isSet) ? jQuery.map(matchedObjects, function(n, i){ if($(n).attr(settings.hook).indexOf(theRel) != -1) return ($(n).attr('data-pretty-share-url')) ? $(n).attr('data-pretty-share-url') : ""; }) : $.makeArray($(this).attr('data-pretty-share-url'));
			// new stuff ends
		...
		*/
		$.prettyPhoto.open = function(event) {
			...
			if(settings.social_tools && pp_data_share_buttons){

				// New stuff
				var pp_data_share_buttons_length = pp_data_share_buttons.length,
					dt_share_html = '',
					dt_is_image = 'image' == _getFileType(pp_images[set_position]);

				for ( var i = 0; i < pp_data_share_buttons_length; i++ ) {
					
					// if image not image - continue
					if ( !dt_is_image && 'pinterest' == pp_data_share_buttons[i] ) {
						continue;
					}

					var dt_item_html = settings.social_tools[ pp_data_share_buttons[i] ].replace('{location_href}', encodeURIComponent(location.href)),
						dt_title = pp_titles[ set_position ],
						dt_img_src = pp_images[ set_position ];

					if ( 'google' == pp_data_share_buttons[i] ) {
						dt_title = dt_title.replace(' ', '+');
					}

					dt_item_html = dt_item_html.replace('{share_title}', dt_title).replace('{image_src}', dt_img_src);
					dt_share_html += dt_item_html;
				}

				$pp_pic_holder.find('.pp_social').html('<div class="entry-share"><div class="soc-ico">' + dt_share_html + '<div></div>');
			}
...
/*			
			if(settings.social_tools && pp_data_share_buttons){
				...
				facebook_like_link = '<div class="entry-share"><div class="soc-ico">' + dt_share_html + '<div><div>'; 
			}
*/
			...
			if(isSet && settings.overlay_gallery) {
				...
				$pp_pic_holder.find('.pp_content').hover( //hoverContainer
					function(e){
						//$pp_pic_holder.find('.pp_gallery:not(.disabled)').fadeIn();
						if (_getFileType(pp_images[set_position])=="image") $pp_pic_holder.find('.pp_gallery').fadeIn();
					},
					function(e){
						// if ( $(e.relatedTarget).parents('.pp_gallery').length > 0 ) return;
