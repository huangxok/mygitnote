$(function () {
    ...
    //////缓冲条开始//已知safari534安卓4.12不兼容537后已可以，
    document.querySelectorAll(".shareIt")[0].onclick = function () {
        var theAudio = document.getElementById("theAudio");
        if (theAudio.buffered) {
            for (p in theAudio.buffered) {
                alert(p);
                alert(theAudio.buffered[p]);
            }
        } else {
            alert(false);
        }
        ...
        //var ee = theAudio.buffered.end(0);
        alert(ee);
        //alert(theAudio.buffered.end(0)+"@"+theAudio.buffered.start(0));

    }
    var t1 = document.getElementById("theAudio");
    t1.addEventListener('progress', function () {//下载数据时触发
        var b = this.buffered;
        var dd = b.start(0);
    });
    //////缓冲条结束
