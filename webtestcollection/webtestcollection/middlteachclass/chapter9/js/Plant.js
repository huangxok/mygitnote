...
Plant = function ( materials, radius ) {
	...
	var segmentAngle = Math.PI * 2 / _branchSegments;
	...
	this.build = function() {
		...
		//for each step
		while (segmentsEachTime < this.totalLinks)	//90
		{
			segmentsEachTime++;
			//last point
			...
			//move forward
			branchPoint.translateZ(this.linkDist);	// 20
			
			this.radius -= radiusStep;	// 30 - 30/90
			...
			//height from transformPoint y - axis
			R = new THREE.Vector3();
			R.cross(transformPoint, diffVector);

			S = new THREE.Vector3();	//x -axis
	...
	this.buildNode = function() {
		...
		while (intSegmentStep < _branchSegments)
		{
			...
			*/
			...
			pX = basePoint.x + transformedRadius * Math.sin(intSegmentStep * segmentAngle) * R.x + transformedRadius * Math.cos(intSegmentStep * segmentAngle) * S.x;
			pY = basePoint.y + transformedRadius * Math.sin(intSegmentStep * segmentAngle) * R.y + transformedRadius * Math.cos(intSegmentStep * segmentAngle) * S.y;
			pZ = basePoint.z + transformedRadius * Math.sin(intSegmentStep * segmentAngle) * R.z + transformedRadius * Math.cos(intSegmentStep * segmentAngle) * S.z;
			
			console.log("R.x="+R.x + " " + R.y + "" + R.z);
			console.log("S.x="+S.x + " " + S.y + "" + S.z);
